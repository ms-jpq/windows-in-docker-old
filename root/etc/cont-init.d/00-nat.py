...
def call_into(
    prog: str, *args: List[str], input: bytes = None, env: Dict[str, str] = None
) -> bytes:
...
def p_non_overlapping(networks: List[IPv4Network]) -> Iterator[IPv4Network]:
    ...
    for exclusion in exclusions:
        if all(not exclusion.overlaps(network) for network in chain(seen, networks)):
...
def rand_mac() -> str:
    def mac_slot():
        return format(randint(0, 255), "02x")
